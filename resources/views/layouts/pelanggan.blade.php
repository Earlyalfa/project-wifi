...
<head>
    ...
    <title>WiFiPay Pelanggan - @yield('title', 'Dashboard Pelanggan')</title>
    ...
    <div class="px-5 py-6 flex items-center gap-3 border-b border-[#968CAA]/30">
        ...
        <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto sidebar-scroll">
            @php
                $menu = [
                    ...
                    ['route' => 'pelanggan.pengaduan', 'icon' => 'message-square-warning', 'label' => 'Pengaduan Gangguan'],
                    ['route' => 'pelanggan.profile', 'icon' => 'user', 'label' => 'Profil Saya'],
                ];
            @endphp
        ...
        <div class="p-3 border-t border-violet-900/50">
            <form action="{{ route('pelanggan.logout') }}" method="POST">
                @csrf
                <button class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-violet-100/60 hover:bg-rose-500/10 hover:text-rose-400 transition-all duration-200">
                        ...
                        <div class="overflow-y-auto flex-1 max-h-[350px]">
                            ...
                            <template x-for="notif in notifications" :key="notif.id">
                                <div @click="markRead(notif)"
                                     :class="{'bg-violet-50/50': !notif.is_read, 'hover:bg-slate-50': true}"
                                     class="flex items-start gap-3 px-4 py-3 border-b border-slate-50 cursor-pointer transition-colors">
                                    {{-- Icon --}}
                                    <div :class="'w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-0.5 ' + (notif.color === 'rose' ? 'bg-rose-100' : notif.color === 'amber' ? 'bg-amber-100' : notif.color === 'emerald' ? 'bg-emerald-100' : 'bg-violet-100')">
                                        <i :data-lucide="notif.icon"
                                           :class="'w-4 h-4 ' + (notif.color === 'rose' ? 'text-rose-600' : notif.color === 'amber' ? 'text-amber-600' : notif.color === 'emerald' ? 'text-emerald-600' : 'text-violet-600')"></i>
                                    </div>
                                    {{-- Content --}}
                                    ...
                                    {{-- Unread dot --}}
                                    <span x-show="!notif.is_read" class="w-2 h-2 rounded-full bg-violet-600 shrink-0 mt-2"></span>
                                </div>
                            </template>
                        </div>
                ...
                <div class="relative" x-data="{ open: false }" x-on:click.outside="open = false">
                    ...
                    <div x-show="open" x-cloak
                        ...
                        <div class="px-4 py-3 border-b border-slate-100">
                            ...
                            <p class="text-xs text-slate-400">{{ auth()->user()->email ?? 'pelanggan@wifiPay.id' }}</p>
                        </div>
<a href="{{ route('pelanggan.profile') }}" class="flex items-center gap-3 px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 transition-colors">
    ...
    <script>
        ...
        function notificationDropdown() {
            return {
                ...
                markRead(notif) {
                    if (!notif.is_read) {
                        fetch('/pelanggan/notifications/' + notif.id + '/read', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })
                            .then(res => res.json())
                            .then(() => {
                                notif.is_read = true;
                ...
                markAllRead() {
                    fetch('/pelanggan/notifications/read-all', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } })
                        .then(res => res.json())
                        .then(() => {
