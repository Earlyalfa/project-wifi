...
@section('content')
    ...
    <form action="{{ url('/login') }}" method="POST" class="flex flex-col gap-4">
        ...
        <div class="text-right -mt-2">
            <a href="#" class="text-xs text-violet-500 hover:underline">Lupa kata sandi?</a>
        </div>

        <button type="submit"
                class="w-full bg-violet-600 hover:bg-violet-700 text-white font-semibold py-3 rounded-full transition mt-2">
            Masuk
        </button>
    ...
    <a href="{{ route('login.google') }}"
       class="w-full flex items-center justify-center gap-3 border border-slate-200 rounded-full px-4 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50 transition mb-4">
        ...
        <p class="text-center text-sm text-slate-600">
            Belum punya akun?
            <a href="{{ route('register') }}" class="text-violet-500 font-semibold hover:underline">Daftar</a>
        </p>
    </form>
@endsection
