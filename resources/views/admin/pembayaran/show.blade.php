...
<div x-data="verifikasiHandler()">
    <a href="{{ route('admin.pembayaran.index') }}" class="flex items-center gap-2 text-slate-500 text-sm mb-4 hover:underline">
            ...
            @if ($pembayaran->status === 'menunggu_verifikasi')
                <div class="bg-white rounded-2xl shadow-sm border border-slate-100 p-5">
                    ...
                    <p class="text-sm text-slate-500 mb-4">
                        Pembayaran ini sedang menunggu verifikasi. Silakan periksa bukti bayar dan lakukan verifikasi.
                    </p>

                    <form action="{{ route('admin.pembayaran.verifikasi', $pembayaran) }}" method="POST" x-ref="formVerifikasi">
                        @csrf
                    ...
                    @if ($pembayaran->status === 'lunas')
                        <div class="bg-emerald-50 rounded-xl p-4 text-center">
                            ...
                            <p class="text-xs text-emerald-500 mt-1">
                                {{ $pembayaran->dibayar_at ? 'Dibayar pada ' . $pembayaran->dibayar_at->format('d M Y H:i') : '' }}
                            </p>
                        </div>
